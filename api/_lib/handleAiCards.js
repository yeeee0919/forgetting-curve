import { createClient } from '@supabase/supabase-js'
import { SYSTEM_PROMPT, ALCHEMIST_SYSTEM_PROMPT } from '../../src/services/cardPrompt.js'
import { validateAiWordList } from '../../src/services/aiWordList.js'
import { parseCardsJson, parseSingleCardJson } from '../../src/services/jsonImport.js'
import { allowAiRate, readInboxInput, readImportText } from './aiGuard.js'

function supabaseUrl() {
    return process.env.SUPABASE_URL || process.env.VITE_SUPABASE_URL || 'https://ttfjdxnasklhealmxgoz.supabase.co'
}

function supabaseAnonKey() {
    return process.env.SUPABASE_ANON_KEY || process.env.VITE_SUPABASE_ANON_KEY || 'sb_publishable_MTX2iPYA2Z52_0Bbt1JZrw_hTFNZODw'
}

function userClient(accessToken) {
    return createClient(supabaseUrl(), supabaseAnonKey(), {
        auth: { persistSession: false, autoRefreshToken: false },
        global: { headers: { Authorization: `Bearer ${accessToken}` } },
    })
}

async function requireUser(accessToken) {
    if (!accessToken) {
        const err = new Error('請先用 Google 登入')
        err.status = 401
        throw err
    }
    const supabase = userClient(accessToken)
    const { data, error } = await supabase.auth.getUser(accessToken)
    if (error || !data?.user) {
        const err = new Error('登入已過期，請再登入一次')
        err.status = 401
        throw err
    }
    return { supabase, user: data.user }
}

function parseQuota(data) {
    if (!data) return { used: 0, limit: 50, remaining: 50 }
    if (typeof data === 'string') return JSON.parse(data)
    return data
}

async function getQuota(supabase) {
    const { data, error } = await supabase.rpc('get_ai_quota')
    if (error) throw new Error(error.message)
    return parseQuota(data)
}

async function consumeQuota(supabase, requested) {
    const { data, error } = await supabase.rpc('consume_ai_quota', { requested })
    if (error) throw new Error(error.message)
    return parseQuota(data)
}

async function refundQuota(supabase, amount) {
    const n = Number(amount)
    if (!Number.isFinite(n) || n < 1) return null
    const { data, error } = await supabase.rpc('refund_ai_quota', { amount: n })
    if (error) {
        console.error('refund_ai_quota:', error.message)
        return null
    }
    return parseQuota(data)
}

function quotaGone(quota) {
    const err = new Error('AI 額度已用完。仍可用「原文 / 譯文」直接加字。')
    err.status = 402
    err.quota = quota
    return err
}

function tooFast() {
    const err = new Error('請求太頻繁，請稍後再試')
    err.status = 429
    return err
}

function badRequest(reason) {
    const err = new Error(reason)
    err.status = 400
    return err
}

async function callOpenAi(messages, temperature = 0.3) {
    const apiKey = process.env.OPENAI_API_KEY
    if (!apiKey) {
        const err = new Error('伺服器尚未設定 OPENAI_API_KEY')
        err.status = 500
        throw err
    }
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${apiKey}`,
        },
        body: JSON.stringify({
            model: 'gpt-4o-mini',
            messages,
            temperature,
            response_format: { type: 'json_object' },
        }),
    })
    if (!response.ok) {
        const errBody = await response.json().catch(() => ({}))
        const err = new Error(errBody?.error?.message || `OpenAI 錯誤 (${response.status})`)
        err.status = 502
        throw err
    }
    const data = await response.json()
    return data.choices?.[0]?.message?.content || '{}'
}

async function reserveThenRun(supabase, reserved, work) {
    const spent = await consumeQuota(supabase, reserved)
    const allowed = Number(spent?.allowed ?? 0)
    if (allowed < 1) throw quotaGone(spent)
    try {
        const result = await work(allowed)
        const used = Number(result.used ?? allowed)
        if (used < allowed) {
            const refunded = await refundQuota(supabase, allowed - used)
            return { ...result, quota: refunded || { ...spent, allowed: used, used: Number(spent.used) - (allowed - used), remaining: Number(spent.remaining) + (allowed - used) } }
        }
        return { ...result, quota: spent }
    } catch (err) {
        await refundQuota(supabase, allowed)
        throw err
    }
}

export async function handleAiCards({ accessToken, body }) {
    const { supabase, user } = await requireUser(accessToken)
    if (!allowAiRate(user.id)) throw tooFast()

    const quota = await getQuota(supabase)
    const remaining = Number(quota?.remaining ?? 0)
    if (remaining <= 0) throw quotaGone(quota)

    const mode = body?.mode || 'import'

    if (mode === 'inbox') {
        const input = readInboxInput(body)
        if (!input.ok) throw badRequest(input.reason)
        const { card, quota: spent } = await reserveThenRun(supabase, 1, async () => {
            const userPrompt = `目標單字：${input.word}\n原始語境：${input.context || '無'}\n字典參考翻譯：${input.hint || '無'}`
            const content = await callOpenAi([
                { role: 'system', content: ALCHEMIST_SYSTEM_PROMPT },
                { role: 'user', content: userPrompt },
            ], 0.2)
            const parsed = parseSingleCardJson(content)
            if (!parsed) {
                const err = new Error('AI 沒有產出有效字卡')
                err.status = 502
                throw err
            }
            return { card: { ...parsed, front: parsed.front || input.word }, used: 1 }
        })
        return { card, quota: spent }
    }

    const input = readImportText(body)
    if (!input.ok) throw badRequest(input.reason)
    const listCheck = validateAiWordList(input.text, { remainingQuota: remaining })
    if (!listCheck.ok) throw badRequest(listCheck.reason || '輸入不符合 AI 匯入格式')

    const { cards, quota: spent } = await reserveThenRun(supabase, listCheck.count, async (allowed) => {
        const content = await callOpenAi([
            { role: 'system', content: SYSTEM_PROMPT },
            { role: 'user', content: input.text },
        ])
        let parsed = parseCardsJson(content)
        if (!parsed.length) {
            const err = new Error('沒有解析到任何單字，請確認格式')
            err.status = 422
            throw err
        }
        parsed = parsed.slice(0, allowed)
        if (!parsed.length) throw quotaGone({ ...quota, remaining: 0 })
        return { cards: parsed, used: parsed.length }
    })
    return { cards, quota: spent }
}

export async function handleGetQuota(accessToken) {
    const { supabase } = await requireUser(accessToken)
    return getQuota(supabase)
}
